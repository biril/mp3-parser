...
(function (globalObject, createModule) {
    ...
    var id3v2TagFrameNames = {
        ...
        readFrameContentT = function (buffer, offset, length) {
            var content = { encoding: buffer.getUint8(offset) };
            content.value = (content.encoding === 0 ? lib.readStr.iso :
                lib.readStr.ucs)(buffer, offset + 1, length - 1);
            return content;
        },
        ...
        readFrameContentTxxx = function  (buffer, offset, length) {
            var
            ...
            offsetTrm = lib.locateStrTrm[enc](buffer, offsetBeg, length - 4);
            if (offsetTrm === -1) {
                return content; // Not found!
            }

            // Read data
            content.description = lib.readStr[enc](buffer, offsetBeg, offsetTrm - offsetBeg);
            offsetTrm += enc === "ucs" ? 2 : 1; // Move past terminating sequence
            content.value = lib.readStr[enc](buffer, offsetTrm, offset + length - offsetTrm);
        ...
        //
        // * URL: a text string
        readFrameContentW = function (buffer, offset, length) {
            return { value: lib.readStr.iso(buffer, offset, length) };
        },
        ...
        // * Description: a text string according to encoding (followed by 00 (00))
        // * URL:         a text string
        readFrameContentWxxx = function (buffer, offset, length) {
            var
                // The content to be returned
                content = { encoding: buffer.getUint8(offset) },
            ...
            content.description = lib.readStr[enc](buffer, offsetBeg, offsetTrm - offsetBeg);
            offsetTrm += enc === "ucs" ? 2 : 1; // Move past terminating sequence
            content.value = lib.readStr.iso(buffer, offsetTrm, offset + length - offsetTrm);
        ...
        readFrameContentComm = function (buffer, offset, length) {
            var
                // The content to be returned
                content = { encoding: buffer.getUint8(offset) },
            ...
            content.language = lib.readTrmStr.iso(buffer, offset + 1, 3);
            ...
            content.description = lib.readStr[enc](buffer, offsetBeg, offsetTrm - offsetBeg);
            offsetTrm += enc === "ucs" ? 2 : 1; // Move past terminating sequence
            content.text = lib.readStr[enc](buffer, offsetTrm, offset + length - offsetTrm);
        ...
        readFrameContentUfid = function (buffer, offset, length) {
            var
                // Read up to the first null terminator to get the owner-identifier
                ownerIdentifier = lib.readTrmStr.iso(buffer, offset, length),
        ...
        readFrameContentIpls = function (buffer, offset, length) {
            var
                // The content to be returned
                content = {
            ...
            while (offsetBeg < offset + length) {
                offsetNextStrTrm = lib.locateStrTrm[enc](buffer, offsetBeg,
                    length - (offsetBeg - offset));
                content.values.push(lib.readStr[enc](buffer, offsetBeg,
        ...
        // * Language:    3 digit (octet) lang-code (ISO-639-2)
        // * Actual text: a text string according to encoding
        readFrameContentUser = function (buffer, offset, length) {
            var
                // The content to be returned
                content = { encoding: buffer.getUint8(offset) },
            ...
            // Read the text field
            content.text = lib.readStr[enc](buffer, offsetBeg, offset + length - offsetBeg);
        ...
        readFrameContentPriv = function (buffer, offset, length) {
            var
                // Read up to the first null terminator to get the owner-identifier
                ownerIdentifier = lib.readTrmStr.iso(buffer, offset, length),

                // Figure out the private data based on frame length vs owner-identifier length
                privateData = new DataView(buffer.buffer, offset + ownerIdentifier.length + 1,
                    length - ownerIdentifier.length - 1);
        ...
        readFrameContentPcnt = function (buffer, offset, length) {
            ...
            return { counter: buffer.getUint32(offset) };
        },
        ...
        // * Counter:       4 octets (at least ..)
        readFrameContentPopm = function (buffer, offset, length) {
            var content = {
                    email: lib.readTrmStr.iso(buffer, offset, length)
                };
            ...
            content.rating = buffer.getUint8(offset);

            // Assume the counter is always exactly 4 octets ..
            content.counter = buffer.getUint32(offset + 1);
        ...
        readFrameContentApic = function (buffer, offset, length) {
            var
                // The content to be returned
                content = { encoding: buffer.getUint8(offset) },
            ...
            offsetTrm = lib.locateStrTrm[enc](buffer, offsetBeg, length - 1);
            if (offsetTrm === -1) {
                return content; // Not found!
            }

            // Read MIME type
            content.mimeType = lib.readStr[enc](buffer, offsetBeg, offsetTrm - offsetBeg);

            // Read picture type
            offsetBeg = offsetTrm + 1;
            content.pictureType = buffer.getUint8(offsetBeg);

            // Locate the the null terminator seperating description and picture data
            offsetBeg += 1;
            offsetTrm = lib.locateStrTrm[enc](buffer, offsetBeg, offset + length - offsetBeg);
            if (offsetTrm === -1) {
                return content; // Not found!
            }
            ...
            // Read picture data
            content.pictureData = new DataView(buffer.buffer, offsetTrm + 1,
                length - offsetTrm + offset - 1);
    ...
    mp3Id3v2Parser.readId3v2TagFrame = function (buffer, offset) {
        ...
        var frame = {
                header: {
                    id: lib.readStr.iso(buffer, offset, 4),
    ...
    mp3Id3v2Parser.readId3v2Tag = function (buffer, offset) {
        ...
        if (!lib.isSeq(lib.seq.id3, buffer, offset)) { return null; }
