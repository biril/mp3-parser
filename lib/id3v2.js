    ...
    var id3v2TagFrameNames = {
        ...
        readFrameContentIpls = function (buffer, offset, length) {
            var
                ...
                // content null-terminator (seperates involvement / involvee)
                offsetNextStrTrm,

                // Choose appropriate string-reader depending on encoding
                readS = isEncodingUcs2 ? lib.readStrUcs2 : lib.readStr,

                // Choose appropriate string-terminator locator depending on encoding
                locateStrTrm = isEncodingUcs2 ? lib.locateStrTrmUcs2 : lib.locateStrTrm;

            offsetBeg = offset + 1;
            while (offsetBeg < offset + length) {
                offsetNextStrTrm = locateStrTrm(buffer, offsetBeg, length - (offsetBeg - offset));
                content.values.push(readS(buffer, offsetBeg, offsetNextStrTrm - offsetBeg));
                offsetBeg = offsetNextStrTrm + (isEncodingUcs2 ? 2 : 1);
            }
        ...
        readFrameContentUser = function (buffer, offset, length) {
            var
                ...
                // Choose appropriate string-reader depending on encoding
                readS = content.encoding === 1 ? lib.readStrUcs2 : lib.readStr;
            ...
            // Read the text field
            content.text = readS(buffer, offsetBeg, offset + length - offsetBeg);
