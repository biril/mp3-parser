...
(function (globalObject, createModule) {
    ...
    */
    ...
    lib.readFrameHeader = function (view, offset) {
        ...
        var b2Str = octetToBinRep(b2);
        var mpegVersion = b2Str.substr(3, 2);
        var layerVersion = b2Str.substr(5, 2);
        header.mpegAudioVersionBits = mpegVersion;
        header.mpegAudioVersion = lib.mpegVersionDescription[mpegVersion];
        header.layerDescriptionBits = layerVersion;
        header.layerDescription = lib.layerDescription[layerVersion];
    ...
    lib.readFrame = function (view, offset, requireNextFrame) {
        ...
        frame._section.sampleLength = lib.sampleLengthMap[frame.header.mpegAudioVersionBits][frame.header.layerDescriptionBits];

        //
        frame._section.byteLength = lib.getFrameByteLength(frame.header.bitrate,
            frame.header.samplingRate, frame.header.framePadding, frame.header.mpegAudioVersionBits, frame.header.layerDescriptionBits);
        ...
        var xingOffset = lib.getXingOffset(frame.header.mpegAudioVersionBits, frame.header.channelModeBits)
